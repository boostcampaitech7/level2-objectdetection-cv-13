...
param_scheduler = [
    dict(
        type='LinearLR',  # 초기 warmup 스케줄러
        start_factor=0.01,  # warmup 시작 비율
        by_epoch=True,
        begin=0,
        end=500,  # warmup_iters에 해당
        convert_to_iter_based=True  # epoch을 iteration으로 변환
    ),
    dict(
        type='StepLR',  # 학습률을 감소시키는 단계적 스케줄러
        begin=0,  # 학습 시작 시점
        end=20,  # 총 학습 epoch 수 (변경 필요 시 조정)
        by_epoch=True,
        step=[9, 15],  # 학습률을 감소시키는 epoch 단계
        gamma=0.1  # 학습률 감소 비율
    )
]
